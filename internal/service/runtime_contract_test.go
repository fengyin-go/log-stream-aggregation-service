...
func TestR010Target(t *testing.T) {
    backend := store.NewRetentionSweepCancellationStore(20 * time.Millisecond)
    coordinator := service.NewRetentionSweepCancellationCoordinator(backend)
    ...
    if !errors.Is(err, context.DeadlineExceeded) || time.Since(started) > 35*time.Millisecond || backend.Calls() != 1 {
        t.Fatalf("保留清理取消没有满足题面描述的触发顺序和公开预期")
    }
}
