...
func TestR030Target(t *testing.T) {
    backend := store.NewSourceRecoveryCancellationStore(20 * time.Millisecond)
    coordinator := service.NewSourceRecoveryCancellationCoordinator(backend)
    ...
    if !errors.Is(err, context.DeadlineExceeded) || time.Since(started) > 35*time.Millisecond || backend.Calls() != 1 {
        t.Fatalf("来源恢复取消没有满足题面描述的触发顺序和公开预期")
    }
}
