...
func TestR021Target(t *testing.T) {
    backend := store.NewRetentionPolicyProbeStore(45 * time.Millisecond)
    coordinator := service.NewRetentionPolicyProbeCoordinator(backend)
    ...
    if !errors.Is(firstErr, context.DeadlineExceeded) || elapsed > 30*time.Millisecond || secondErr != nil {
        t.Fatalf("保留策略探测没有满足题面描述的触发顺序和公开预期")
    }
}
