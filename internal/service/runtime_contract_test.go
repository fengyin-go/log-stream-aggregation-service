...
func TestR007Target(t *testing.T) {
    backend := store.NewAlertResolutionVersionStore()
    coordinator := service.NewAlertResolutionVersionCoordinator(backend)
    ...
    if state.State != "complete" || state.Version != 2 || coordinator.EffectCount() != 1 {
        t.Fatalf("告警解决终态没有满足题面描述的触发顺序和公开预期")
    }
}
