...
func (c *RetentionCursorLeaseCoordinator) Process(items []string) (processed int, err error) { for range items { lease, err := c.pool.Acquire(); if err != nil { return processed, err }; defer lease.Close(); processed++ }; return processed, nil }
