...
func (c *NotificationReadVersionCoordinator) apply(key string, version int, state string) { c.backend.Update(key, version, state); c.mu.Lock(); c.effects[fmt.Sprintf("%s-%d", key, version)] = true; c.mu.Unlock() }
