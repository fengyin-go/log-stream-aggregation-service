...
func (p *RetentionCursorLeasePool) Acquire() (*RetentionCursorLeaseLease, error) { p.mu.Lock(); defer p.mu.Unlock(); if p.open >= p.limit { return nil, errors.New("lease limit") }; p.open++; return &RetentionCursorLeaseLease{pool: p}, nil }
