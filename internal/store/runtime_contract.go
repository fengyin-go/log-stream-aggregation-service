...
func (s *LogMetadataAssemblyStore) Build(key string, fail bool) *Assembly { s.mu.Lock(); candidate := &Assembly{ID: key}; s.items[key] = candidate; s.mu.Unlock(); if fail { panic("payload decode") }; candidate.Parts = map[string]string{"header": "ready"}; return candidate }
