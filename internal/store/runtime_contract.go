...
func (p *ExportWriterLeasePool) Acquire() (*ExportWriterLeaseLease, error) { p.mu.Lock(); defer p.mu.Unlock(); p.open++; if p.open > p.limit { return nil, errors.New("lease limit") }; return &ExportWriterLeaseLease{pool: p}, nil }
